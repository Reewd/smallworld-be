...
func SeedDemoData(ctx context.Context, users Users, verifications Verifications, vehicles Vehicles) error {
	...
	if err := users.Save(ctx, domain.User{
		...
		Preferences: domain.UserPreferences{
			WalkToPickup:       domain.PreferenceLevelBig,
			WalkFromDropoff:    domain.PreferenceLevelBig,
			DriverPickupDetour: domain.PreferenceLevelBig,
		},
		CreatedAt: now,
	}); err != nil {
		return err
	...
	if err := users.Save(ctx, domain.User{
		...
		Preferences: domain.UserPreferences{
			WalkToPickup:       domain.PreferenceLevelBig,
			WalkFromDropoff:    domain.PreferenceLevelBig,
			DriverPickupDetour: domain.PreferenceLevelLow,
		},
		CreatedAt: now,
	}); err != nil {
		return err
